...
function Controller(props) {
    ...
    // opacity relevant
    const fadingIn = useMemo(() => new Value(0), []);
    const fadingOut = useMemo(() => new Value(0), []);
    // gesture relevant
    ...
    const opacity = useMemo(() => {
        ...
        return block([
            cond(fadingIn, [
                ...
                cond(not(clockRunning(fadeInClock)), [
            ...
            cond(fadingOut, [
                ...
                cond(not(clockRunning(fadeOutClock)), [
        ...
        return block([
            ...
            cond(eq(state, State.END), [
                cond(and(not(clockRunning(transitionClock)), neq(transitionState.finished, 1)), [
                    ...
                    set(snapY, translateY),
                    set(endX, translateX),
                    cond(lessThan(translateY, boundaryTop + 40), set(endY, boundaryTop), cond(greaterThan(add(translateY, CONTROLLER_WIDTH), viewportHeight - boundaryBottom - 40), set(endY, viewportHeight - boundaryBottom - CONTROLLER_WIDTH), 
                    // Y the screens main place
                    [
                        cond(lessThan(velocityY, -2000), set(endY, boundaryTop), cond(greaterThan(velocityY, 2000), set(endY, viewportHeight - boundaryBottom - CONTROLLER_WIDTH), set(endY, translateY))),
    ...
    // ]), []);
    return (<Animated.View style={[styles.container, { opacity, transform: [{ translateX }, { translateY }] }]}>
